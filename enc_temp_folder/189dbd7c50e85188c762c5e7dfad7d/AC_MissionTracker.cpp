...
void UAC_MissionTracker::NextObjective()
{
	if (HasMission && CurrentMission.Objectives.Num() > 0 && IsValid(CurrentMission.Objectives[0].ObjectiveTrackingActor))
	{
		...
		IObjectiveTracker* objectiveTracker = Cast<IObjectiveTracker>(CurrentMission.Objectives[0].ObjectiveTrackingActor);
		...
		else
		{
			...
			{
				objectiveTracker = Cast<IObjectiveTracker>(components[i]);
				if (objectiveTracker)
				{
